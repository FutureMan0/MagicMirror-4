class UntisModule {
  constructor(config = {}) {
    this.config = {
      ...
      // Filter für Wahlpflichtfächer: nur diese Fächer anzeigen (leer = alle)
      // Beispiel: ['w-el'] zeigt nur Elektrotechnik-Wahlpflicht
      wahlpflichtFilter: config.wahlpflichtFilter || [],
      // Wenn true, werden bei mehreren Stunden im gleichen Slot nur die erste angezeigt
      compactMode: config.compactMode !== false
    };

    this.translations = {
      'de': {
        loading: 'Lade Stundenplan...',
        retry: 'Versuch ${attempt} fehlgeschlagen, warte 2s...',
        next_attempt: 'Nächster Versuch in 15 min',
        empty: 'Keine Stunden gefunden',
        error: 'Fehler',
        days: ['Mo', 'Di', 'Mi', 'Do', 'Fr'],
        today: 'Heute',
        next_lesson: 'Nächste Stunde',
        no_more_lessons: 'Keine weiteren Stunden'
      },
      'en': {
        loading: 'Loading timetable...',
        retry: 'Attempt ${attempt} failed, waiting 2s...',
        next_attempt: 'Next attempt in 15 min',
  ...
  async render() {
    ...
    if (!this.active) return this.container;
    ...
    this.updateInterval = setInterval(() => {
      if (this.active) {
        this.loadTimetableWithRetry(2).then(() => {
          if (this.active) this.renderView();
        });
      }
    }, 900000);
  ...
  async loadTimetableWithRetry(maxRetries = 3) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      ...
      if (attempt < maxRetries && this.active) {
  ...
  async loadTimetable() {
    try {
      ...
      const rawTimetable = data.result || [];
      ...
      // Hochwertige Deduplizierung basierend auf normalisiertem Inhalt
      const seen = new Set();
      this.timetable = rawTimetable.filter(lesson => {
        const info = this.getLessonInfo(lesson);
        // Normalisiere Werte für den Schlüssel
        ...
        const room = String(info.room || '').trim();
        const teach = String(info.teacher || '').trim();

        const key = `${date}|${start}|${end}|${subj}|${room}|${teach}`;
      ...
      console.log(`Untis: ${this.timetable.length} Stunden geladen (nach Deduplizierung)`);
  ...
  renderWeekView() {
    ...
    // Immer 5 Tage (Mo-Fr) anzeigen
    const daysToShow = 5;

    let html = '<div class="untis-week-view">';
    
    // Dynamisches Grid-Template für Header und Grid
    const gridTemplate = `45px repeat(${daysToShow}, 1fr)`;
    
    html += `<div class="untis-header" style="grid-template-columns: ${gridTemplate};">`;

    // Leere Zelle für Zeit-Spalte
    html += '<div class="untis-header-time"></div>';

    // Header mit Tagen (Mo-Fr)
    for (let i = 0; i < daysToShow; i++) {
      const date = new Date(weekStart);
      date.setDate(weekStart.getDate() + i);
      date.setHours(0, 0, 0, 0);
      const dateDayOfWeek = date.getDay();
      const dayIndex = dateDayOfWeek === 0 ? 6 : dateDayOfWeek - 1; // Montag = 0, Sonntag = 6
    ...
    // Stunden-Grid mit dynamischen Spalten
    html += `<div class="untis-grid" style="grid-template-columns: ${gridTemplate};">`;
    ...
    timeSlots.forEach((slot, slotIndex) => {
      ...
      // Stunden für jeden Tag (nur heute bis Freitag)
      for (let day = 0; day < daysToShow; day++) {
        ...
        // Vergangene Tage komplett ausgrauen
        if (date.getTime() < todayTime) {
          isPastSlot = true;
        }
        // Am heutigen Tag: Stunden ausgrauen die komplett vorbei sind
        else if (date.getTime() === todayTime) {
          const nextSlot = timeSlots[slotIndex + 1];
          if (nextSlot) {
            // Es gibt einen nächsten Slot: aktueller Slot ist vorbei wenn nextSlot begonnen hat
            ...
            // Letzter Slot: vorbei wenn 50 Minuten nach Startzeit vergangen sind
            isPastSlot = nowMinutes >= slotMinutes + 50;
          }
        }

        if (lessons.length > 0) {
          // Filtere Gruppen-Fächer wenn compactMode aktiv ist
          let filteredLessons = lessons;
          
          if (this.config.compactMode && lessons.length > 1) {
            const lessonInfos = lessons.map(l => ({ lesson: l, info: this.getLessonInfo(l) }));
            
            // Gruppiere nach Basis-Fachname
            const bySubject = {};
            lessonInfos.forEach(({ lesson, info }) => {
              const subjectLower = info.subject.toLowerCase();
              // Wahlpflicht (w-el, w-gb, etc.) alle als "wahlpflicht" gruppieren
              let baseSubject;
              if (subjectLower.startsWith('w-')) {
                baseSubject = 'wahlpflicht';
              } else {
                // Basis-Fachname (z.B. "LA" statt "LA W103")
                baseSubject = info.subject.split(' ')[0].toLowerCase();
              }
              if (!bySubject[baseSubject]) bySubject[baseSubject] = [];
              bySubject[baseSubject].push({ lesson, info });
            });
            
            // Für jede Fachgruppe: nur eine Stunde behalten
            filteredLessons = [];
            Object.entries(bySubject).forEach(([key, group]) => {
              if (group.length === 1) {
                filteredLessons.push(group[0].lesson);
              } else {
                // Mehrere Gruppen: nur erste behalten (oder gefilterte)
                let selected = group[0];
                
                // Wahlpflicht-Filter prüfen
                if (key === 'wahlpflicht' && this.config.wahlpflichtFilter.length > 0) {
                  const filtered = group.find(({ info }) => 
                    this.config.wahlpflichtFilter.some(f => 
                      info.subject.toLowerCase().includes(f.toLowerCase())
                    )
                  );
                  if (filtered) selected = filtered;
                }
                
                filteredLessons.push(selected.lesson);
              }
            });
          }
          ...
          html += `<div class="untis-empty-slot${isPastSlot ? ' untis-past' : ''}"></div>`;
        }
      }
    });
